function [ corr_matches ] = sad( to_be_matched, I1, I2, win_x, win_y)
% Find correspondences using sad given features in image 1,
% image 1, and image 2.  win_x and win_y are the matching window's
% width and height
...
for i=1:match_count(1)
    ...
    curr_sad = 5000; % this sets the threshold for finding a sad/ssd/ncc
    ...
    for m = win_x+1:img_size(2);
        ...
        sad_value = sum(sum(sum(abs(int16(mat1) - int16(mat2)))));
        if (sad_value < curr_sad)
            curr_sad = sad_value;
            location = [m y];
        end;
    end;
